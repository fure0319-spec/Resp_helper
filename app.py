...
if os.path.exists(CRITERIA_FILE):
    try:
        ...
        if all_dfs:
            ...
            # 2. 불필요한 열 삭제
            cols_to_drop = ['질환', '선정기준', 'Disease', 'Selection Criteria']
            ...
            with c2:
                # [핵심] 너비 조절을 위한 옵션
                fit_width = st.checkbox("화면 너비에 꽉 채우기", value=True, 
                                      help="체크를 해제하면 가로 스크롤이 생기며, 각 컬럼의 너비를 마우스로 자유롭게 조절할 수 있습니다.")
            ...
            # [핵심] 기본 너비를 픽셀 단위로 넉넉하게 지정
            col_config = {
                cols[0]: st.column_config.Column(width="small"),    # 분류
                cols[1]: st.column_config.Column(width="medium"),   # A열
                cols[2]: st.column_config.Column(width=350),        # B열 (350픽셀)
                cols[3]: st.column_config.Column(width=450),        # C열 (450픽셀)
            ...
            st.dataframe(
                styled_df, 
                use_container_width=fit_width, # 이 값이 False면 너비 조절 가능
                hide_index=True,
