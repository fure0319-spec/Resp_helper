    ...
    try:
        ...
        for sheet in target_sheets:
            try:
                ...
                if col_title: new_row['연구 과제'] = df_sheet[col_title[0]]
                elif len(df_sheet.columns) > 1: new_row['연구 과제'] = df_sheet.iloc[:, 1]
                else: new_row['연구 과제'] = ""

                col_selection = [c for c in df_sheet.columns if "선정" in c]
                if col_selection: new_row['선정 기준'] = df_sheet[col_selection[0]]
                elif len(df_sheet.columns) > 2: new_row['선정 기준'] = df_sheet.iloc[:, 2]
                else: new_row['선정 기준'] = ""

                col_exclusion = [c for c in df_sheet.columns if "제외" in c]
                if col_exclusion: new_row['제외 기준'] = df_sheet[col_exclusion[0]]
                elif len(df_sheet.columns) > 3: new_row['제외 기준'] = df_sheet.iloc[:, 3]
                else: new_row['제외 기준'] = ""
        ...
        if all_dfs:
            ...
            with col_mode:
                view_mode = st.radio("보기 방식 선택", 
                                     ["전체 내용 보기 (줄바꿈/자동높이)", "너비 조절 모드 (드래그 가능)"],
                                     horizontal=True)
            ...
            if view_mode == "전체 내용 보기 (줄바꿈/자동높이)":
                # [모드 1] HTML 방식: 내용 다 보임, 너비는 코드로 고정 (선정/제외 균등 분배)
                st.markdown("""
                <style>
                    .styled-table {
                        width: 100%;
                    ...
                    .styled-table td {
                        ...
                        word-wrap: break-word; /* 단어 끊기 허용 */
                        background-color: white;
                        color: black;
                    }
                    /* 컬럼 너비 강제 조정 (합계 100%) */
                    .styled-table th:nth-child(1), .styled-table td:nth-child(1) { width: 10%; } /* 분류 */
                    .styled-table th:nth-child(2), .styled-table td:nth-child(2) { width: 20%; } /* 연구과제 */
                    .styled-table th:nth-child(3), .styled-table td:nth-child(3) { width: 35%; } /* 선정기준 */
                    .styled-table th:nth-child(4), .styled-table td:nth-child(4) { width: 35%; } /* 제외기준 */
                </style>
                ...
                # [모드 2] Dataframe 방식: 드래그 가능, 내용은 잘릴 수 있음
                st.info("💡 이 모드에서는 컬럼 사이의 구분선을 마우스로 잡고 너비를 자유롭게 조절할 수 있습니다.")
                st.dataframe(
                    df_display, 
                    use_container_width=False, # 가로 스크롤 및 너비 조절 허용
                    hide_index=True,
                    height=500
                )
