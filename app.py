    ...
    try:
        ...
        for sheet in target_sheets:
            try:
                ...
                # 컬럼 이름 공백 제거 (매칭률 높이기 위해)
                raw_df.columns = [str(c).strip() for c in raw_df.columns]
                
                new_row = {}
                new_row['분류'] = sheet
                
                # [수정] 강력한 컬럼 찾기 (이름으로 찾고, 없으면 순서로 강제 지정)
                # B열: 연구 과제 (index 1)
                col_title = [c for c in raw_df.columns if "과제" in c or "제목" in c]
                if col_title: new_row['연구 과제'] = raw_df[col_title[0]]
                elif len(raw_df.columns) > 1: new_row['연구 과제'] = raw_df.iloc[:, 1]
                else: new_row['연구 과제'] = ""

                # C열: 선정 기준 (index 2)
                col_selection = [c for c in raw_df.columns if "선정" in c]
                if col_selection: new_row['선정 기준'] = raw_df[col_selection[0]]
                elif len(raw_df.columns) > 2: new_row['선정 기준'] = raw_df.iloc[:, 2]
                else: new_row['선정 기준'] = ""

                # D열: 제외 기준 (index 3)
                col_exclusion = [c for c in raw_df.columns if "제외" in c]
                if col_exclusion: new_row['제외 기준'] = raw_df