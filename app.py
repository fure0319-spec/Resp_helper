...
# ==========================================
# [추가 기능] 하단에 상세 엑셀 파일(선정/제외 기준) 표시
# ==========================================
st.markdown("---")
st.header("📑 연구별 상세 선정/제외 기준 (Reference)")

# 업로드한 상세 엑셀 파일 이름
CRITERIA_FILE = "criteria.xlsx"  # 1단계에서 올린 파일명과 똑같아야 합니다!
...
if os.path.exists(CRITERIA_FILE):
    try:
        ...
        # 1. 검색 기능 (첫번째 열 기준)
        search_query = st.text_input("🔍 연구명 또는 키워드 검색", placeholder="예: COPD, 천식, 녹농균...")
        ...
        # 2. 화면에 표로 보여주기
        # (첫번째 열과 선정/탈락 기준 등 모든 열이 보입니다)
        st.dataframe(
            df_display, 
            use_container_width=True, 
            hide_index=True  # 0,1,2... 같은 불필요한 인덱스 번호 숨김
        )
        st.caption(f"💡 표의 헤더를 클릭하면 정렬할 수 있고, 오른쪽 위 확대 버튼으로 크게 볼 수 있습니다.")

    except Exception as e:
        st.error(f"상세 엑셀 파일을 읽는 중 오류가 났습니다: {e}")
else:
    st.info("ℹ️ 상세 기준 파일(criteria.xlsx)이 아직 업로드되지 않았습니다.")