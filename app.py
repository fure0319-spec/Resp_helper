    ...
    try:
        ...
        for row in ws.iter_rows(min_row=1, values_only=True):
            if row[0] and len(row) > 1:
                ...
                # [상단 박스용] Markdown 줄바꿈 처리 (공백 2칸 + 엔터)
                val = val.replace('\r\n', '\n').replace('\n', '  \n')
                data[key] = val
        wb.close()
...
if os.path.exists(CRITERIA_FILE):
    try:
        ...
        if all_dfs:
            ...
            # [수정] 데이터프레임 스타일 적용 (Arial, 10pt, 서식 없음)
            # Pandas Styler를 사용하여 CSS 속성을 직접 주입합니다.
            styled_df = df_display.style.set_properties(**{
                'font-family': 'Arial',
                'font-size': '10pt',
                'text-align': 'left',
                'background-color': 'white', # 배경색 흰색 고정 (서식 제거 효과)
                'color': 'black',            # 글자색 검정
                'white-space': 'pre-wrap',   # 엑셀 내 줄바꿈 반영
