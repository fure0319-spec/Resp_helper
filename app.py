...
if os.path.exists(CRITERIA_FILE):
    try:
        ...
        if all_dfs:
            ...
            # -----------------------------------------------------------------
            # [UI 배치] 검색창 | 보기 방식 | 글자 크기
            # -----------------------------------------------------------------
            col_search, col_mode, col_font = st.columns([3, 2, 1])
            ...
            with col_font:
                # [추가됨] 글자 크기 조절 슬라이더
                font_size = st.slider("글자 크기 (pt)", min_value=10, max_value=16, value=10, step=1)
            ...
            if view_mode == "전체 내용 보기 (줄바꿈/자동높이)":
                ...
                <style>
                    .styled-table {{
                        ...
                        font-size: {font_size}pt; /* 글자 크기 반영 */
                        table-layout: fixed;
                    }}
                    .styled-table th {{
                    ...
                    .styled-table th:nth-child(3), .styled-table td:nth-child(3) {{ width: 35%; }}
                ...
                # [모드 2] Dataframe 방식 (글자 크기 변수 적용)
                st.info("💡 컬럼 경계선을 마우스로 드래그하여 너비를 조절하세요.")
                
                # Pandas Styler로 폰트 크기 주입
                styled_df = df_display.style.set_properties(**{
                    'font-family': 'Arial',
                    'font-size': f'{font_size}pt', # 글자 크기 반영
                    'text-align': 'left',
                    'white-space': 'pre-wrap', 
                    'vertical-align': 'top'
                })
